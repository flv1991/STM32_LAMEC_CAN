...
void TM_ILI9341_Init() {
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_SPI1, ENABLE);   // liga o spi2 

  GPIO_Lcd.GPIO_Pin = ILI9341_WRX_PIN;  //
  GPIO_Lcd.GPIO_Mode = GPIO_Mode_Out_PP;

  GPIO_Lcd.GPIO_Speed = GPIO_Speed_50MHz;

  GPIO_Init(ILI9341_WRX_PORT, &GPIO_Lcd);
	
  GPIO_Lcd.GPIO_Pin = ILI9341_CS_PIN;
	GPIO_Init(ILI9341_CS_PORT, &GPIO_Lcd);
	GPIO_Lcd.GPIO_Pin = ILI9341_RST_PIN;
	GPIO_Init(ILI9341_RST_PORT, &GPIO_Lcd);
	...
	GPIO_Spi.GPIO_Pin = GPIO_Pin_5| GPIO_Pin_6 | GPIO_Pin_7;
	...
	//GPIO_PinAFConfig(GPIOE, GPIO_PinSource6, GPI O_AF_SPI4);
...
void drawbmp(const uint16_t *bmp ,TM_ILI9341_Image_t modo
	,uint16_t xinit, uint16_t yinit, uint16_t xend, uint16_t yend ){
	
	uint16_t x,y,largura,altura;
	uint32_t i=0;
	
	
	if (modo==TM_ILI9341_Image_Horizontal)
	{
	    ...
	    for( x = yinit; x <  yend; ++x )
      {
				for( y = xinit; y < xend; ++y )
        {
					TM_ILI9341_DrawPixel(y,  x, bmp[i]);
					i++;
				}
			}
