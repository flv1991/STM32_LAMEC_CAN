...
//ativar o cs PD2
	...
	sprintf(buffer,"CS");
	DRAW_DisplayString( 1, 115, buffer, strlen(buffer));
...
SPI_I2S_SendData(SPI2, address);
	...
	sprintf(buffer,"add");
	DRAW_DisplayString( 5, 105, buffer, strlen(buffer));
...
SPI_I2S_SendData(SPI2, data);
	sprintf(buffer,"data");
	DRAW_DisplayString( 20, 95, buffer, strlen(buffer));
//Esperar pela info
while(!SPI_I2S_GetFlagStatus(SPI2, SPI_I2S_FLAG_RXNE));
ReceiveData=SPI_I2S_ReceiveData(SPI2);
	sprintf(buffer,"ReceiveData");
	DRAW_DisplayString( 5, 85, buffer, strlen(buffer));
//desativar o cs PD
GPIO_WriteBit(GPIOD, GPIO_Pin_2, Bit_SET); 
sprintf(buffer,"notCS");
DRAW_DisplayString( 1, 75, buffer, strlen(buffer));
sprintf(buffer2,"addr2:%d",ReceiveData);
DRAW_DisplayString( 1, 20, buffer2, strlen(buffer2));
		return (ReceiveData);
...
//Transfer the received data from the data register
	DRAW_Init ();
//configacc();
addr=SPI_send(0x20, 0b11000111);
addr=SPI_send(0x21, 0b00000000);
addr=SPI_send(0x22, 0b00000000);
sprintf(buffer2,"addr1:%d",addr);
DRAW_DisplayString( 1, 4, buffer2, strlen(buffer2));

addr=SPI_send(0x0f, 0x00); 


sprintf(buffer2,"addr2:%d",addr);
DRAW_DisplayString( 1, 18, buffer2, strlen(buffer2));
LCD_FillRect(0, 0, 128, 128, 0xffff);
    while (1)
    {   
		i++;
sprintf(buffer2,"cnt:%d",i);
DRAW_DisplayString( 1, 4, buffer2, strlen(buffer2));
